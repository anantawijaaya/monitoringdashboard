...
<div id="submitBudgetModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4 transition-all">
    <div class="bg-white rounded-1xl shadow-2xl border border-slate-100 max-w-2xl w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="bg-[#121212] px-6 py-4 text-white flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <h3 id="modalProgramTitle" class="text-base font-extrabold text-white">Direct Selling</h3>
                </div>
            </div>
        ...
        <!-- Modal Form Body -->
        <form id="submitBudgetForm" action="{{ route('budget-bk.direct-sales.expense.store') }}" method="POST" enctype="multipart/form-data" onsubmit="return validateBudgetForm(event)" class="flex-1 overflow-y-auto p-6 space-y-5">
            @csrf
            <input type="hidden" id="inputProgramName" name="program_name" value="Direct Selling">
            ...
            <!-- Note / Catatan Perbaikan Field (Khusus Role USER / ADMIN) -->
