                        ...
                        <td class="py-3.5 px-4 whitespace-nowrap">
                            @if($item->evidence_path)
                                <a href="{{ asset('storage/' . $item->evidence_path) }}" target="_blank" class="inline-flex items-center gap-1.5 text-[#ED1C24] hover:text-[#C8102E] font-bold cursor-pointer">
                                    ...
                                    <span>{{ $item->evidence_original_name ?? 'bukti_invoice.pdf' }}</span>
                                </a>
                            @else
                        ...
                        <td class="py-3.5 px-4 whitespace-nowrap">
                             @if(Auth::check() && Auth::user()->isAdmin())
                                 <div class="flex items-center gap-1.5">
                                     <form action="{{ route('budget-bk.indirect-channel.expense.status', $item->id) }}" method="POST" class="inline">
                                         @csrf
                        ...
                        <td class="py-3.5 px-4 text-right whitespace-nowrap">
                             <div class="flex items-center justify-end gap-1.5">
                                 ...
                                 @if(Auth::check() && Auth::user()->isAdmin())
                                     ...
                                     <form action="{{ route('budget-bk.indirect-channel.expense.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data pengajuan ini?')" class="inline">
                                         @csrf
                                         @method('DELETE')
