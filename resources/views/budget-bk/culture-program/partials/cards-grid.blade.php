<!-- PROGRAM CULTURE CARDS SECTION (WHITE CONTAINER) -->
<div class="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4">
    <div>
        <h2 class="text-[19px] font-extrabold text-slate-900 tracking-tight">Culture Program</h2>
        ...
        @foreach($programCardsList as $card)
            ...
            <div class="bg-[#121212] border border-gray-800 hover:border-gray-700 rounded-2xl p-4 text-center flex flex-col justify-between transition-all hover:shadow-xl group">
                <div>
                    ...
                    <h3 class="text-[14px] font-bold text-white leading-snug h-8 flex items-center justify-center" title="{{ $card['title'] }}">
                        {{ $card['title'] }}
                    </h3>
