...
class DirectSalesImportService
{
    ...
    public function processExtractedRows(array $rows, string $mode = 'replace'): array
    {
        ...
        for ($r = 0; $r < min(10, count($rows)); $r++) {
            ...
            foreach ($normalizedCandidate as $nh) {
                if (
                    ...
                    str_contains($nh, 'direct') ||
                    str_contains($nh, 'selling') ||
                    str_contains($nh, 'grebek') ||
                    str_contains($nh, 'poi') ||
                    str_contains($nh, 'dls') ||
                    str_contains($nh, 'skul')
                ) {
                    $headerRowIndex = $r;
                    $headers = $candidate;
                    break 2;
        ...
        foreach ($headers as $idx => $hStr) {
            ...
            elseif (str_contains($nh, 'direct') || str_contains($nh, 'selling')) $map['direct_selling'] = $idx;
            elseif (str_contains($nh, 'grebek') || str_contains($nh, 'poi') || str_contains($nh, 'site')) $map['grebek_poi'] = $idx;
            elseif (str_contains($nh, 'dls') || str_contains($nh, 'skul')) $map['dls_skulid'] = $idx;
        }

        if ($map['cluster'] === -1) $map['cluster'] = 0;
        if ($map['mitra'] === -1) $map['mitra'] = 1;
        if ($map['total_budget'] === -1) $map['total_budget'] = 2;
        if ($map['direct_selling'] === -1) $map['direct_selling'] = 3;
        if ($map['grebek_poi'] === -1) $map['grebek_poi'] = 4;
        if ($map['dls_skulid'] === -1) $map['dls_skulid'] = 5;
        ...
        try {
            if ($mode === 'replace') {
                DirectSalesAllocation::query()->delete();
            }
            ...
            foreach ($dataRows as $row) {
                ...
                $ds = $this->cleanNumeric((string) ($row[$map['direct_selling']] ?? '0'));
                $gp = $this->cleanNumeric((string) ($row[$map['grebek_poi']] ?? '0'));
                $dls = $this->cleanNumeric((string) ($row[$map['dls_skulid']] ?? '0'));
                ...
                if (($ds + $gp + $dls) <= 0.0) {
                    $pDs = $this->cleanNumeric((string) ($row[3] ?? '0'));
                    $pGp = $this->cleanNumeric((string) ($row[4] ?? '0'));
                    $pDls = $this->cleanNumeric((string) ($row[5] ?? '0'));

                    if (($pDs + $pGp + $pDls) > 0.0) {
                        $ds = $pDs; $gp = $pGp; $dls = $pDls;
                    }
                }
                ...
                if ($total > 0.0 && ($ds + $gp + $dls) <= 0.0) {
                    $evenShare = round($total / 3, 2);
                    $ds = $evenShare;
                    $gp = $evenShare;
                    $dls = $total - ($evenShare * 2);
                }
                ...
                $recordsToInsert[] = [
                    ...
                    'total_budget' => $total,
                    'direct_selling_budget' => $ds,
                    'grebek_poi_budget' => $gp,
                    'dls_skulid_budget' => $dls,
                    'created_at' => $now,
                    'updated_at' => $now,
                ];
            ...
            if (!empty($recordsToInsert)) {
                foreach (array_chunk($recordsToInsert, 500) as $chunk) {
                    DB::table('direct_sales_allocations')->insert($chunk);
                }
            }
            ...
            throw new Exception("Gagal menyimpan data alokasi direct sales: " . $e->getMessage());
        }
    }
}
